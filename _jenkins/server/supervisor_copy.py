...
def finish_with_failure(serverID,parent_dict,exception, tl_name, user_info):
    parent_dict[serverID]['busy'] = False
    send_information(id,tl_name,user_info,test_passed="UNKNOWN_FAIL")
    sys.exit()
...
def main(serverID, reservation_data, parent_dict, user_info, jenkins_info, reservationID = None):
    ...
    if has_got_fail:
        for test in range(0,len(job_test_status_dict)):
             if remove_tag_from_file(tl_address = tl_address, folder_name=jenkins_info['parameters']['name'],
